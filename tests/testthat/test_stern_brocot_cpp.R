...
test_that("stern_brocot_cpp handles scalar inputs correctly", {
  x <- 1.667
  result <- stern_brocot_cpp(x, GABOR_UNCERTAINTY, GABOR_UNCERTAINTY)
...
test_that("stern_brocot_cpp handles vector x with scalar uncertainties", {
  x <- c(1.667, sqrt(2))
  result <- stern_brocot_cpp(x, GABOR_UNCERTAINTY, GABOR_UNCERTAINTY)
...
test_that("stern_brocot_cpp handles vector x with vector uncertainties", {
  x <- c(1.667, sqrt(2))
  lower_uncertainty <- c(0.067, 0.2)
  upper_uncertainty <- c(0.133, 0.4)
  result <- stern_brocot_cpp(x, lower_uncertainty, upper_uncertainty)
...
test_that("stern_brocot_cpp errors on mismatched vector lengths", {
  x <- c(1.667, sqrt(2))
  lower_uncertainty <- c(0.067, 0.068, 0.065)
  upper_uncertainty <- c(0.133, 0.4)
  expect_error(
    stern_brocot_cpp(x, lower_uncertainty, upper_uncertainty),
    "lower_uncertainty must either be of length 1 or match the length of x"
  )
})

# Test for invalid uncertainties
test_that("stern_brocot_cpp errors on invalid uncertainties", {
  x <- c(1.667, sqrt(2))
...
test_that("close to 0.5 returns 1/2 with symmetrical uncertainty", {
  x <- 0.49
  uncertainty <- 0.03
  result <- stern_brocot_cpp(x, uncertainty, uncertainty)
